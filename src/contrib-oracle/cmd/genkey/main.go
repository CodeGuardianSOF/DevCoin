...
func main() {
    ...
    if err := os.WriteFile(privPath, []byte(enc(priv)), 0o600); err != nil { panic(err) }
    if err := os.WriteFile(seedPath, []byte(enc(seed)), 0o600); err != nil { panic(err) }
    if err := os.WriteFile(pubPath, []byte(enc(pub)), 0o644); err != nil { panic(err) }
