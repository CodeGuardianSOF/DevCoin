...
func TestPRMerged_MintsWithAuthHeader(t *testing.T) {
	...
	if res != "minted" {
		t.Fatalf("expected minted, got %s", res)
	}
	if atomic.LoadInt32(calls) != 1 {
		t.Fatalf("expected 1 call, got %d", atomic.LoadInt32(calls))
	}
	if *auth != "Bearer test-token" {
		t.Fatalf("missing/incorrect auth header: %s", *auth)
	}
	if (*body)["proposer"].(string) != "authority1" {
		t.Fatalf("bad proposer: %v", (*body)["proposer"])
	}
	if (*body)["to"].(string) != "charlie" {
		t.Fatalf("bad to: %v", (*body)["to"])
	}
...
func TestDocsPR_MintsReducedReward(t *testing.T) {
	...
	if (*body)["amount"].(float64) != 25 {
		t.Fatalf("bad amount for docs PR: %v", (*body)["amount"])
	}
}
...
func TestPushMain_NonMaintainer_Rewarded(t *testing.T) {
	...
	if (*body)["amount"].(float64) != 50 {
		t.Fatalf("bad amount for push: %v", (*body)["amount"])
	}
}
