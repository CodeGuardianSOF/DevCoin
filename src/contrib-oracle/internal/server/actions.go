	...
	case "pull_request":
		if pr, ok := obj["pull_request"].(map[string]any); ok {
			...
			if state == "closed" && merged {
				...
				if user != "" && amount > 0 {
					if err := s.mintFor(user, amount); err == nil {
						return "minted"
					}
				}
			}
	...
	case "issues":
		if action, _ := obj["action"].(string); action == "closed" {
			if issue, ok := obj["issue"].(map[string]any); ok {
				...
				if user == "" {
					user = extractLogin(issue, "user")
				}
				if user != "" {
					if err := s.mintFor(user, 25); err == nil {
						return "minted"
					}
				}
			}
	...
	case "pull_request_review":
		if review, ok := obj["review"].(map[string]any); ok {
			...
			if strings.EqualFold(state, "approved") {
				...
				if user != "" {
					if err := s.mintFor(user, 10); err == nil {
						return "minted"
					}
				}
			}
					...
					if cmt, ok := ci.(map[string]any); ok {
						...
						if author == "" {
							if committer, ok := cmt["committer"].(map[string]any); ok {
								if u, ok := committer["username"].(string); ok {
									author = u
								}
							}
						}
						if author == "" || s.isMaintainer(author) {
							continue
						}
						...
						if err := s.mintFor(author, 50); err == nil {
							minted = true
						}
					}
				}
...
func hasDocLabel(pr map[string]any) bool {
	...
	if t, ok := pr["title"].(string); ok {
		...
		if strings.Contains(lt, "[docs]") || strings.Contains(lt, "docs:") {
			return true
		}
	}
	return false
